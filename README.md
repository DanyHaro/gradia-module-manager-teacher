  ...
  DB_HOST=dpg-d4f53bili9vc739cgsng-a.oregon-postgres.render.com
  DB_PORT=5432
  ...
  ### Cursos (2 endpoints - Solo lectura)

  ⚠️ **IMPORTANTE:** Los cursos se crean manualmente en la BD por el ADMIN. Los docentes solo pueden ver cursos donde están inscritos.

  ```
  GET    /api/cursos                      - Obtener mis cursos inscritos como docente
  GET    /api/cursos/:id                  - Obtener detalle de un curso (si estoy inscrito)
  ```
  ...
  **Solución:** El middleware `authenticate` no se está aplicando correctamente. Verifica que las rutas estén protegidas en `app.js` o en los archivos de rutas.
  ...
  - **Total de Endpoints:** ~58 funcionales
  ...
  2. Ver mis cursos inscritos → `GET /api/cursos`
  3. Crear unidades en mis cursos → `POST /api/unidades`
  ...
  ⚠️ **NOTA:** Los cursos son creados manualmente en la BD por el ADMIN, quien inscribe a los docentes y estudiantes.
